...
export class CatatanPage implements OnInit{
 ...
 this.router.navigateByUrl('/', { replaceUrl: true }); // alihkan ke halama
  ...
  setTimeout(() => {
    ...
    handler: (data: any) => {
    //jika tekan yakin
    this._apiService.hapus(id, '/hapusCatatan.php?id=').subscribe({
    next: (res: any) => {
    console.log('sukses', res);
    this.page = 0;
    this.perPage = 10;
